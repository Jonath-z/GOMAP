...
const Options = () => {
  ...
  return (
    <div ref={ref}>
      <CgMenuGridR
        className="text-white text-4xl cursor-pointer absolute left-4"
        onClick={openSlideOptions}
      />
      <div className="absolute top-10 xsm:left-2 mt-20"></div>
      {isSlideOptions && (
        <div className="absolute z-10 left-0 pb-5 w-max pr-5 pl-5 pt-5 bg-white top-10 sm:mt-5">
          <div>
            <button
              ...
              className="w-56 ml-2 h-8 hover:bg-gray-300 text-gray-700"
              onClick={() => {
                setDisplay("none");
          ...
          {isContributeFormOpened && (
            <div>
              <ContributeForm cancel={cancel} />
            </div>
          )}
        </div>
      )}
